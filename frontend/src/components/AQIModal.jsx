...
import { X, AlertCircle, CheckCircle, Heart, Home, Baby, Leaf, Wind } from "lucide-react";
...
const AQIModal = ({ isOpen, onClose, level, aqiInfo }) => {
  ...
  const aqiDetailInfo = {
    1: {
      description: "Kualitas udara sangat baik dan tidak menimbulkan risiko kesehatan.",
      details: "Udara bersih dengan konsentrasi polutan yang sangat rendah. Kondisi ideal untuk semua aktivitas outdoor.",
      effects: [
      ...
      recommendations: "Manfaatkan kualitas udara yang baik untuk beraktivitas di luar ruangan. Ini adalah waktu terbaik untuk berolahraga, berjalan-jalan, atau aktivitas outdoor lainnya."
    },
    2: {
      description: "Kualitas udara dapat diterima untuk sebagian besar orang, namun beberapa individu sensitif mungkin mengalami gejala ringan.",
      details: "Tingkat polusi udara masih dalam batas aman, tetapi mulai terdeteksi peningkatan konsentrasi polutan tertentu.",
      effects: [
      ...
      recommendations: "Aktivitas outdoor masih aman untuk sebagian besar orang. Namun, jika Anda memiliki kondisi pernapasan sensitif, pertimbangkan untuk mengurangi aktivitas outdoor yang intens."
    },
    3: {
      description: "Anggota kelompok sensitif dapat mengalami efek kesehatan. Masyarakat umum kemungkinan tidak akan terpengaruh.",
      details: "Konsentrasi polutan mulai meningkat dan dapat mempengaruhi kesehatan kelompok rentan seperti anak-anak, lansia, dan penderita penyakit pernapasan.",
      effects: [
      ...
      recommendations: "Kelompok sensitif sebaiknya membatasi waktu di luar ruangan. Tutup jendela untuk mencegah polusi masuk. Gunakan air purifier di dalam rumah jika tersedia."
    },
    4: {
      description: "Setiap orang dapat mulai mengalami efek kesehatan. Anggota kelompok sensitif dapat mengalami efek kesehatan yang lebih serius.",
      details: "Tingkat polusi udara berbahaya dan dapat mempengaruhi kesehatan semua orang, tidak hanya kelompok sensitif. Paparan berkepanjangan dapat menyebabkan masalah kesehatan serius.",
      effects: [
      ...
      recommendations: "Batasi semua aktivitas outdoor. Kelompok sensitif harus tetap di dalam ruangan dengan jendela tertutup. Gunakan masker N95 jika terpaksa keluar. Hindari olahraga outdoor sepenuhnya."
    },
    5: {
      description: "Peringatan kesehatan: semua orang dapat mengalami efek kesehatan yang serius. Ini adalah kondisi darurat kesehatan masyarakat.",
      details: "Tingkat polusi sangat berbahaya. Seluruh populasi lebih mungkin terpengaruh secara serius. Kondisi ini memerlukan tindakan darurat dan perlindungan maksimal.",
      effects: [
      ...
      recommendations: "DARURAT KESEHATAN! Jangan keluar rumah kecuali sangat mendesak. Gunakan masker N95 atau P100 jika terpaksa keluar. Pantau berita dan informasi dari otoritas kesehatan. Pertimbangkan untuk meninggalkan area jika memungkinkan."
    }
  };
      ...
      >
        ...
        >
          ...
          <div className="p-6 md:p-8">
            {/* Header */}
            <div className={`bg-gradient-to-r ${aqiInfo.color} rounded-2xl p-6 mb-6`}>
              <div className="flex items-center space-x-4">
                <span className="text-6xl">{aqiInfo.emoji}</span>
                <div>
                  <div className="flex items-center space-x-3 mb-2">
                    <span className="text-5xl font-black text-white">{level}</span>
                    <h2 className="text-3xl font-bold text-white">{aqiInfo.label}</h2>
                  </div>
            ...
            {/* Detail Kondisi */}
            <div className={`${aqiInfo.bgColor} border ${aqiInfo.borderColor} rounded-2xl p-6 mb-6`}>
              <h3 className={`text-xl font-bold ${aqiInfo.textColor} mb-3 flex items-center space-x-2`}>
                <AlertCircle className="h-6 w-6" />
                <span>Detail Kondisi</span>
              </h3>
              <p className="text-slate-300 leading-relaxed">{detailInfo.details}</p>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {detailInfo.effects.map((effect, idx) => (
                  ...
                  >
                    <div className={`w-2 h-2 rounded-full ${aqiInfo.bgColor} mt-2 flex-shrink-0`}></div>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {detailInfo.actions.map((action, idx) => (
                  ...
                  >
                    ...
                    <p className="text-slate-300 text-sm leading-relaxed">{action.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Rekomendasi */}
            <div className={`${aqiInfo.bgColor} border-l-4 ${aqiInfo.borderColor} rounded-r-2xl p-6`}>
              <h3 className={`text-lg font-bold ${aqiInfo.textColor} mb-3`}>
                💡 Rekomendasi Utama
              </h3>
              <p className="text-slate-300 leading-relaxed">{detailInfo.recommendations}</p>
            </div>

            {/* Footer Note */}
            <div className="mt-6 bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
              <p className="text-slate-400 text-xs leading-relaxed text-center">
                ⚠️ Informasi ini bersifat umum. Jika Anda memiliki kondisi kesehatan khusus atau mengalami gejala yang mengkhawatirkan,
