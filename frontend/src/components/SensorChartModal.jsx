...
const SensorChartModal = ({ isOpen, onClose, sensorType, currentValue, icon, color }) => {
      ...
      >
        ...
        >
          ...
          <div className="p-6 md:p-8">
            {/* Header */}
            <div className={`bg-gradient-to-r ${sensorInfo.color} rounded-2xl p-6 mb-6`}>
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center space-x-4">
                  <div className="text-5xl">{icon}</div>
                  <div>
                    <h2 className="text-3xl font-bold text-white mb-1">{sensorInfo.title}</h2>
                    <p className="text-white/90 text-lg">Data Historis & Analisis</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-white/80 text-sm mb-1">Nilai Saat Ini</p>
                  <p className="text-4xl font-black text-white">{currentValue}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className={`${sensorInfo.bgColor} border ${sensorInfo.borderColor} rounded-2xl p-5`}>
              ...
              <div className={`${sensorInfo.bgColor} border ${sensorInfo.borderColor} rounded-2xl p-5`}>
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <p className="text-sm text-slate-400 font-medium">Rata-rata</p>
                </div>
              ...
              <div className={`${sensorInfo.bgColor} border ${sensorInfo.borderColor} rounded-2xl p-5`}>
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <p className="text-sm text-slate-400 font-medium">Total Data</p>
                </div>
                <p className="text-3xl font-black text-white">{chartData.length}</p>
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold text-white">Grafik Tren Data</h3>
                ...
                <svg className="w-full h-full pl-8" viewBox="0 0 800 300" preserveAspectRatio="none">
                  ...
                  <defs>
                    <linearGradient id={`gradient-${sensorType}`} x1="0%" y1="0%" x2="0%" y2="100%">
                      <stop offset="0%" style={{stopColor: sensorInfo.lineColor, stopOpacity: 0.3}} />
                      <stop offset="100%" style={{stopColor: sensorInfo.lineColor, stopOpacity: 0}} />
                    </linearGradient>
                  </defs>
                  ...
                  {/* Area under line */}
                  <path
                    d={`M 0 300 ${chartData.map((point, i) => {
                      const x = (i / (chartData.length - 1)) * 800;
                      const y = 300 - ((point.value - minValue) / (maxValue - minValue)) * 300;
                      return `L ${x} ${y}`;
                    }).join(" ")} L 800 300 Z`}
            ...
            <div className="mt-6 bg-slate-900/60 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50">
              <h3 className="text-xl font-bold text-white mb-4">Tabel Data Detail</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-700">
                      <th className="text-left py-3 px-4 text-slate-400 font-semibold">Tanggal</th>
                      <th className="text-left py-3 px-4 text-slate-400 font-semibold">Waktu</th>
                      <th className="text-right py-3 px-4 text-slate-400 font-semibold">Nilai ({sensorInfo.unit})</th>
                      <th className="text-right py-3 px-4 text-slate-400 font-semibold">Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {chartData.slice().reverse().slice(0, 10).map((point, i) => (
                      <tr key={i} className="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                        <td className="py-3 px-4 text-slate-300">{point.date}</td>
                        <td className="py-3 px-4 text-slate-300">{point.time}</td>
                        <td className="py-3 px-4 text-right font-bold text-white">{point.value}</td>
                        <td className="py-3 px-4 text-right">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                            point.value > stats.avg * 1.1 ? "bg-red-500/20 text-red-400" :
                            point.value < stats.avg * 0.9 ? "bg-blue-500/20 text-blue-400" :
                            "bg-green-500/20 text-green-400"
                          }`}>
                            {point.value > stats.avg * 1.1 ? "Tinggi" :
                             point.value < stats.avg * 0.9 ? "Rendah" : "Normal"}
                          </span>
                        </td>
                      </tr>
                    ))}
